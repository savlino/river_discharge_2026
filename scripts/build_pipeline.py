...
from ingest_niwis import load_all_niwis_csv
from ingest_pegelonline import validate_niwis_against_pegelonline_api
from ingest_hydro_eaufrance import load_all_hydro_eaufrance_csv
from ingest_hubeau import validate_hydro_eaufrance_against_hubeau
...
def _write_csv_if_allowed(df: pd.DataFrame, path) -> None:
    """Preserves existing CSV snapshots unless overwrite is explicitly enabled."""
    if path.exists() and not OVERWRITE_EXISTING_CSV:
...
def run_pipeline():
    ...
    # 1. Ensure output directories exist; NIWIS raw exports must be
    ...
    df_germany = load_all_niwis_csv(RAW_NIWIS_DIR)
    ...
    df_validation_de = validate_niwis_against_pegelonline_api(df_germany)
    if not df_validation_de.empty:
        validation_output_path = OUTPUT_DIR / "pegelonline_validation.csv"
        _write_csv_if_allowed(df_validation_de, validation_output_path)
    ...
    df_france = load_all_hydro_eaufrance_csv(RAW_HYDRO_EAUFRANCE_DIR)
    print(f"-> Ingested {len(df_france)} rows for French stations.")

    # 3b. Validate against Hub'Eau recent-observations API (rolling ~1-month window); archives
    ...
    print("\n[4/6] Validating against Hub'Eau recent-observations API (rolling ~1-month window)...")
    df_validation_fr = validate_hydro_eaufrance_against_hubeau(df_france)
    if not df_validation_fr.empty:
        validation_output_path = OUTPUT_DIR / "hydro_eaufrance_validation.csv"
        _write_csv_if_allowed(df_validation_fr, validation_output_path)
    ...
    _write_csv_if_allowed(df_tidy, tidy_output_path)
    ...
    _write_csv_if_allowed(df_corr, corr_output_path)
    ...
    _write_csv_if_allowed(df_summary, summary_output_path)
