...
The independent API checks broadly support the manual exports. Full-month PEGELONLINE and Hub'Eau snapshots were recovered and are stored in `output/pegelonline_validation.csv` and `output/hydro_eaufrance_validation.csv`. Across the full PEGELONLINE month (31 days per station), the maximum absolute difference is **3.0 cm at Kaub** and **3.4 cm at Dresden** between the NIWIS daily value and the PEGELONLINE daily mean. For Hub'Eau, coverage runs 2026-08-03 through 2026-08-31 for Tonneins (29 days) and Ternay (29 days), and 2026-08-03 through 2026-08-27 for Blois (25 days, matching its shorter manual export); the maximum absolute difference is **2.9 mm at Tonneins** and **0.0 mm at both Blois and Ternay**.

The validation CSVs also include `relative_diff_pct`, calculated as `diff / abs(manual_value) * 100`. This expresses the API-minus-manual discrepancy relative to the manual observation while remaining meaningful for French stations whose gauge values can be negative. Over the full August window, the largest PEGELONLINE relative differences are **10.0% at Kaub** (on 2026-08-01, when the absolute gauge value was smallest) and **3.82% at Dresden**. The Hub'Eau report shows a maximum relative difference of **0.34% at Tonneins** and **0.0% at both Blois and Ternay**. Relative percentages should still be read alongside the absolute difference: near-zero gauge readings can make a small absolute discrepancy look large in relative terms.

The near-zero French discrepancies are notably tighter than the German ones, and this is itself a meaningful data point rather than a sign of superior measurement precision. NIWIS and PEGELONLINE are genuinely separate German operators/systems, so their small but real disagreement reflects two independent measuring chains. The Hydro-Eaufrance manual export and the Hub'Eau recent-observations API both draw from France's shared Système d'Information sur l'Eau, so the near-perfect match likely indicates the same underlying measurement (or a very similar aggregation of it) accessed through two interfaces, rather than two independent instruments agreeing. Treat the French cross-check as confirming internal consistency of the SIE pipeline, not as independent validation of the measurement itself.

Hub'Eau recent-observations validation covers the dates available in both the recovered snapshot and the manual French exports. These comparisons validate agreement in the overlapping period; the manual exports remain the authoritative source for the summer dataset and historical baselines.
...
1. **PEGELONLINE REST API (`pegelonline.wsv.de`) / Hub'Eau recent-observations API**
   - *Technical Limitation:* Both are open, no-auth REST APIs, but each enforces a rolling window on raw time-series data -- PEGELONLINE exposes roughly the most recent month, while Hub'Eau's official `observations_tr` endpoint rejects any `date_debut_obs` older than ~1 calendar month. Full-August snapshots for both APIs were captured while the recent window still covered them and are preserved in `output/pegelonline_validation.csv` and `output/hydro_eaufrance_validation.csv`.
